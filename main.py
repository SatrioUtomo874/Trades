...
client = Spot(api_key="YOUR_API_KEY", api_secret="YOUR_API_SECRET", base_url="https://api.binance.com")
...
def find_swing(df, left=2, right=2):
    swing_high = []
    swing_low = []
...
def get_support_resistance(client, symbol: str, interval: str = "1h", lookback: int = 50):
    ...
    Params:
        symbol: nama koin, contoh "BTCUSDT"
        interval: timeframe, contoh "1h", "4h", "1d"
        lookback: jumlah candle untuk analisis
    Returns:
...
def get_trade_levels_with_percent(client, symbol: str, interval: str = "15m", buffer=0.001, sl_buffer=0.0015):
    ...
    Params:
        buffer: entry offset dari harga saat ini (0.1% default)
        sl_buffer: SL offset (0.2% default)
    Return:
        dict: entry_price, take_profit, stop_loss, tp_percent, sl_percent, direction
    """
...
load_dotenv()
BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN")  # Token dari BotFather
...
async def coin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.args:
        ...
        support = get_support_resistance(client, symbol, "4h")["support"]
        resistance = get_support_resistance(client, symbol, "4h")["resistance"]
        ...
        # 4️⃣ Volume (dibandingkan rata-rata historis)
        min_volume = get_volume(client, symbol, "4h", limit=20) * 0.8
        if volume >= min_volume:  # 20% di atas rata-rata
            confidence += 20
...
# trend = get_trend(client, "BTCUSDT", "4h")
# rsi = get_rsi(client, "BTCUSDT", "4h")
# smc = get_trend_smc(client, "BTCUSDT", "4h")
