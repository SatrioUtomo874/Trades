...
MIN_RR = 2.0
MAX_RR = 4.0   # Batas maksimum RR (akan di-cap jika level memberikan RR > 4.0)
...
# ============================================================
# SESSION MIN CONFIDENCE DINONAKTIFKAN (SEMUA 0)
# ============================================================
SESSION_MIN_CONF = {
    "Asia": 0,
    "London": 0,
...
def score_direction(df_h1, df_m15, df_d1=None):
    ...
    # Penalti jika setup berlawanan dengan bias H1
    ...
    if in_killzone:
        if setup_bull > setup_bear:
            setup_bull *= KILLZONE_BOOST
        else:
            setup_bear *= KILLZONE_BOOST
...
def _collect_entry_candidates(m15, direction, entry_ref, atr):
    ...
    # Jika tidak ada kandidat, berikan market entry
    if not cands:
...
# ============================================================
# PEMILIHAN TP TERBAIK (DENGAN RR 2.0 - 4.0)
# ============================================================

def _select_best_tp(tp_pool, entry_price, risk, max_rr=MAX_RR):
    if not tp_pool:
        return None, None

    qualifying = []  # RR antara MIN_RR dan max_rr
    too_far = []     # RR > max_rr
    ...
    if qualifying:
        # Pilih yang tier terendah (prioritas) dan RR terkecil
        best = min(qualifying, key=lambda x: (x[2], x[3]))
        lbl, v, tier, rr = best
        return round(v, 8), lbl

    if too_far:
        # Ambil yang RR terkecil di antara yang terlalu jauh
...
def analyze_setup(df_h1, df_m15, direction, entry_price, score=None, invalid_level=None):
    ...
    tp_price, tp_label = _select_best_tp(tp_pool, entry_price, risk, max_rr=MAX_RR)

    # Fallback jika tidak ada level sama sekali: buat TP dengan MIN_RR
    if tp_price is None:
    ...
    if rr < MIN_RR:
        return None  # Seharusnya tidak terjadi, tapi jaga-jaga
...
def full_analyze(df_h1, df_m15, df_d1=None, symbol=None):
    try:
        ...
        # Session filter sudah dinonaktifkan (semua 0), jadi tidak akan menolak
        # Tapi tetap kita cek untuk kompatibilitas
        bar_ts = score.get("bar_ts")
        session = _get_session(bar_ts)
        min_conf = SESSION_MIN_CONF.get(session, 0)
        if confidence < min_conf:
            return None

        # Filter NY (saya nonaktifkan sementara agar tidak membunuh sinyal)
        # if session == "NY":
        #     choch_ok = ...
        #     if not (choch_ok or fr_ok): return None
        ...
        # Cegah TP sudah terlewati dari harga saat ini
