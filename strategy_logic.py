"""
strategy_logic.py — OTAK v4 (Revisi: Inducement, External Liquidity, Trail Struktural)
========================================================================================
...
TRAIL_R_LADDER = []  # v4: adaptive trend/RSI trail; no fixed profit ladder.
...
# =============================================================================
# ADAPTIVE TRAIL ENGINE v4
# =============================================================================
# Trail bukan lagi "latest swing = geser SL".
# Semua metrik dihitung dari M15 OHLCV yang SUDAH tersedia.
TRAIL_MIN_ACTIVATION_R = 0.90       # jangan kunci profit kecil terlalu dini
TRAIL_SOFT_ACTIVATION_R = 1.15     # mulai memakai structure secara normal
TRAIL_DECAY_TRIGGER = 0.68         # strength di bawah ini = trend mulai lemah
TRAIL_STRONG_THRESHOLD = 0.78
TRAIL_VERY_STRONG_THRESHOLD = 0.88
TRAIL_MIN_LOCK_R = 0.18             # lock minimum setelah activation
TRAIL_MAX_LOCK_R = 0.62             # jangan mengejar SL terlalu dekat
TRAIL_SWING_LB = 3
TRAIL_LOOKBACK = 60
TRAIL_BUFFER_ATR_WEAK = 0.42
TRAIL_BUFFER_ATR_NORMAL = 0.55
TRAIL_BUFFER_ATR_STRONG = 0.72
TRAIL_BUFFER_ATR_VERY_STRONG = 0.90

def _clip01(x: float) -> float:
    return max(0.0, min(1.0, float(x)))

def _trend_strength_components(df: pd.DataFrame, direction: str,
                               lookback: int = TRAIL_LOOKBACK) -> dict:
    """Measure M15 trend health, not merely HH/HL existence.

    Components:
      - directional efficiency: net movement / total movement
      - impulse quality: recent bodies relative to ATR
      - pullback quality: depth of recent counter-move
      - EMA slope/alignment
      - RSI level + RSI slope
      - RSI divergence penalty
      - structure quality

    Returns normalized 0..1 components and a 0..100 strength score.
    """
    if df is None or len(df) < 35:
        return {"strength": 0.5, "regime": "unknown"}

    d = df.tail(max(35, lookback)).copy()
    ...
    # Pullback depth: compare the latest counter-move to the latest impulse.
    w = d.tail(24)
    sh, sl = swing_pts(d, lb=TRAIL_SWING_LB)
    ...
    if direction == "bull" and sl and sh:
        ...
        depth = _clip01(pullback / max(impulse_leg, atr_now))
        pullback_quality = 1.0 - depth
    ...
    ema_align = _clip01(
        0.5
        + sign * (ema9 - ema21) / max(atr_now * 2.0, 1e-10)
    ...
    # Strong trend can stay overbought/oversold. Penalize only when RSI is
    # actually decaying, not merely because it is extreme.
    ...
    divergence_penalty = 0.18 if div.get("strong") else (0.10 if (
        div.get("bull_div") if direction == "bull" else div.get("bear_div")
    ) else 0.0)
    ...
    raw = (
        ...
        + 0.14 * structure_quality
        + 0.15 * ema_quality
        + 0.20 * r_momentum
    )
    strength = _clip01(raw - divergence_penalty)

    # Require a meaningful directional RSI/price context before calling it strong.
    if r_level < 0.35:
        strength *= 0.90

    if strength >= TRAIL_VERY_STRONG_THRESHOLD:
        regime = "very_strong"
    elif strength >= TRAIL_STRONG_THRESHOLD:
        regime = "strong"
    elif strength >= TRAIL_DECAY_TRIGGER:
        regime = "healthy"
    elif strength >= 0.48:
        regime = "transition"
    else:
    ...
    return {
        ...
        "rsi_level": round(r_level, 4),
        "rsi_momentum": round(r_momentum, 4),
        "rsi": round(r_now, 2),
        "rsi_slope": round(r_slope, 2),
        "rsi_divergence": bool(div.get("bull_div") if direction == "bull" else div.get("bear_div")),
        "atr": atr_now,
    }

def adaptive_trailing_stop(df_m15: pd.DataFrame, entry: float,
                           current_sl: float, direction: str,
                           initial_risk: float, current_price: float,
                           tp: Optional[float] = None) -> dict:
    """Return an adaptive trailing candidate.

    The function is deliberately conservative before ~0.9R so fees/noise do
    not turn small floating profits into frequent tiny exits. Once active,
    trend strength controls how much room the latest confirmed structure gets.
    RSI is a momentum/decay sensor, never a standalone exit trigger.
    """
    if df_m15 is None or len(df_m15) < 35 or initial_risk <= 0:
        return {"candidate": None, "strength": 0.0, "regime": "unknown", "reason": "insufficient_data"}
    ...
    pnl_r = sign * (float(current_price) - float(entry)) / max(initial_risk, 1e-10)
    metrics = _trend_strength_components(d, direction)
    strength = metrics["strength"]
    regime = metrics["regime"]

    # No early micro-locks. A winner needs enough room to cover fees/slippage.
    if pnl_r < TRAIL_MIN_ACTIVATION_R:
        return {**metrics, "candidate": None, "reason": "below_activation"}

    # Base profit protection rises slowly with R, but is never allowed to
    # become the primary exit while trend strength is high.
    if pnl_r < TRAIL_SOFT_ACTIVATION_R:
        base_lock = TRAIL_MIN_LOCK_R
    else:
        # 0.18R at 1.15R -> ~0.45R at 2R -> cap 0.62R.
        base_lock = min(TRAIL_MAX_LOCK_R, 0.18 + 0.27 * (pnl_r - TRAIL_SOFT_ACTIVATION_R))

    # Strong/very-strong trends get a wider structural buffer and a smaller
    # lock; decay/weak trends get a tighter buffer and preserve more profit.
    if regime == "very_strong":
        buffer_atr = TRAIL_BUFFER_ATR_VERY_STRONG
        lock_scale = 0.65
    elif regime == "strong":
        buffer_atr = TRAIL_BUFFER_ATR_STRONG
        lock_scale = 0.78
    elif regime == "healthy":
        buffer_atr = TRAIL_BUFFER_ATR_NORMAL
        lock_scale = 0.92
    elif regime == "transition":
        buffer_atr = TRAIL_BUFFER_ATR_NORMAL * 0.82
        lock_scale = 1.05
    else:
        buffer_atr = TRAIL_BUFFER_ATR_WEAK
        lock_scale = 1.18

    desired_lock_r = min(TRAIL_MAX_LOCK_R, max(TRAIL_MIN_LOCK_R, base_lock * lock_scale))
    floor_sl = float(entry) + sign * desired_lock_r * initial_risk

    sh, sl_pts = swing_pts(d, lb=TRAIL_SWING_LB)
    structural = None
    atr_now = metrics["atr"]
    if direction == "bull" and sl_pts:
        swing_low = float(d["low"].iloc[sl_pts[-1]])
        structural = swing_low - buffer_atr * atr_now
    elif direction == "bear" and sh:
        swing_high = float(d["high"].iloc[sh[-1]])
        structural = swing_high + buffer_atr * atr_now

    # We want the most protective level that still respects the current trend.
    # In a strong trend, structure dominates and floor only protects against
    # catastrophic profit giveback. In decay, the floor becomes increasingly
    # important.
    if structural is None:
        proposed = floor_sl
    elif direction == "bull":
        proposed = max(structural, floor_sl)
    else:
        proposed = min(structural, floor_sl)

    # Never loosen the existing SL and never place it beyond TP.
    if direction == "bull":
        proposed = max(float(current_sl), proposed)
        if tp is not None:
            proposed = min(proposed, float(tp) - max(atr_now * 0.05, 1e-12))
    else:
        proposed = min(float(current_sl), proposed)
        if tp is not None:
            proposed = max(proposed, float(tp) + max(atr_now * 0.05, 1e-12))

    improves = proposed > current_sl if direction == "bull" else proposed < current_sl
    reason = f"{regime}|strength={strength:.2f}|r={pnl_r:.2f}|rsi={metrics.get('rsi',0):.1f}|rsi_slope={metrics.get('rsi_slope',0):+.2f}"
    return {
        **metrics,
        "candidate": float(proposed) if improves else None,
        "proposed": float(proposed),
        "pnl_r": round(pnl_r, 3),
        "lock_r": round(desired_lock_r, 3),
