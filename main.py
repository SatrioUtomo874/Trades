...
try:
    ...
    # .env adalah sumber konfigurasi canonical. Override=True memastikan .env
    # mengalahkan nilai warisan dari trades.env yang mungkin sudah dimuat launcher.
    load_dotenv(os.path.join(BASE_DIR, ".env"), override=True)
    # Backward compatibility only: load trades.env without overriding .env.
    if not os.environ.get("TELEGRAM_BOT_TOKEN") and not os.environ.get("TELEGRAM_TOKEN"):
        load_dotenv(os.path.join(BASE_DIR, "trades.env"), override=False)
except ImportError:
    pass  # environment variables exported manually tetap berfungsi
...
class Config:
    ...
    telegram_bot_token: str = field(default_factory=lambda: os.environ.get("TELEGRAM_BOT_TOKEN") or os.environ.get("TELEGRAM_TOKEN", ""))
    telegram_chat_id: str = field(default_factory=lambda: os.environ.get("ALLOWED_USER_ID") or os.environ.get("TELEGRAM_CHAT_ID", ""))
    ...
    def validate_telegram(self) -> List[str]:
        ...
        if not self.telegram_bot_token:
            missing.append("TELEGRAM_BOT_TOKEN / TELEGRAM_TOKEN")
...
def setup_logging(state_dir: str) -> logging.Logger:
    # Runtime directories are deliberately separate from source code.
    base = os.path.abspath(os.path.join(BASE_DIR, state_dir))
    ...
    if getattr(root, "_smc_configured", False):
        return root
    ...
    for filename, level in (("main.log", logging.DEBUG), ("errors.log", logging.WARNING)):
        handler = logging.handlers.RotatingFileHandler(
            os.path.join(logs_dir, filename), maxBytes=10_000_000, backupCount=5, encoding="utf-8"
        )
        handler.setLevel(level)
        handler.setFormatter(fmt)
        root.addHandler(handler)
    root._smc_configured = True
    return root
...
class BinanceClient:
    ...
    def get_order(self, symbol: str, order_id: Any) -> Any:
    ...
    def close_position_market(self, symbol: str, position_amt: float) -> Any:
        qty = abs(Decimal(str(position_amt)))
        if qty <= 0:
            return None
        side = "SELL" if position_amt > 0 else "BUY"
        return self._request(
            "POST", "/fapi/v1/order",
            {"symbol": symbol, "side": side, "type": "MARKET", "quantity": str(qty), "reduceOnly": "true"},
            signed=True,
        )
...
class StateStore:
    ...
    def __init__(self, checkpoint_path: str):
        ...
        self.bans: Dict[str, Dict[str, Any]] = {}  # symbol -> {reason, expiry, permanent}
    ...
    def manual_cancel(self, symbol: str) -> bool:
        """Administrative cleanup path; never records a trading outcome."""
        with self.symbol_lock(symbol):
            with self._lock:
                pos = self.positions.get(symbol)
                if not pos:
                ...
                pos["close_reason"] = "MANUAL_TIMEOUT"
    ...
    def ban(self, symbol: str, reason: str, duration_sec: Optional[float] = None, permanent: bool = False) -> None:
        with self._lock:
            self.bans[symbol] = {
                "reason": reason,
                "expiry": None if permanent else time.time() + float(duration_sec or 0),
    ...
    def is_banned(self, symbol: str) -> bool:
        with self._lock:
            ...
            if b.get("permanent"):
                return True
            if b.get("expiry") is None or b["expiry"] <= time.time():
    ...
    def cleanup_expired_bans(self) -> List[str]:
        ...
        with self._lock:
            for symbol in list(self.bans.keys()):
                if self.bans[symbol].get("permanent"):
                    continue
                if self.bans[symbol].get("expiry") is None or self.bans[symbol]["expiry"] <= time.time():
                    del self.bans[symbol]
...
class TradingBot:
    def __init__(self, cfg: Config):
        self.cfg = cfg
        state_dir = cfg.state_dir if os.path.isabs(cfg.state_dir) else os.path.join(BASE_DIR, cfg.state_dir)
        os.makedirs(state_dir, exist_ok=True)
        self.state = StateStore(os.path.join(state_dir, "main_checkpoint.json"))
        self.strategy_engine = strategy.Strategy()
        self.learn_engine = learn.LearnEngine(
            checkpoint_path=os.path.join(state_dir, "learn_checkpoint.json"),
        ...
        self._protection_queue: "queue.Queue[str]" = queue.Queue()
        self._protection_queued: set[str] = set()
        self._stop = threading.Event()
    ...
    def _check_pending_fill(self, symbol: str, pos: Dict[str, Any], price: float, ts: float) -> None:
        ...
        # In REAL mode, a Bybit price touch is only a fill candidate. Confirm
        # that Binance has actually filled the tracked limit order / position.
        if self.state.mode == "REAL":
            entry_order_id = (pos.get("binance_order_ids") or {}).get("entry")
            try:
                if entry_order_id is not None:
                    order = self.binance.get_order(symbol, entry_order_id)
                    status = str(order.get("status", "")).upper()
                    if status not in {"FILLED", "PARTIALLY_FILLED"}:
                        return
                if not self.binance.get_position_risk(symbol):
                    return
            except RateLimitError as e:
                self._enter_binance_pause(str(e))
                return
            except ExchangeError as e:
                logger.warning("Fill confirmation %s gagal: %s", symbol, e)
                return
    ...
    def _attach_real_protection(self, symbol: str, pos: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not confirmed:
                logger.warning("Posisi %s belum terkonfirmasi di Binance, masuk protection queue", symbol)
                self._enqueue_protection(symbol)
                return
    ...
    def _handle_timeout(self, symbol: str, pos: Dict[str, Any]) -> None:
        ...
        if hasattr(self.learn_engine, "record_timeout_event"):
            try:
                self.learn_engine.record_timeout_event(pos, reason="AUTOMATIC_TIMEOUT")
            except TypeError:
                try:
                    self.learn_engine.record_timeout_event(pos, "AUTOMATIC_TIMEOUT")
                except Exception:
                    logger.exception("record_timeout_event gagal untuk %s", symbol)
            except Exception:
                logger.exception("record_timeout_event gagal untuk %s", symbol)
        elif hasattr(self.learn_engine, "record_timeout"):
            try:
                self.learn_engine.record_timeout(pos)
            except Exception:
                logger.exception("record_timeout gagal untuk %s", symbol)
    ...
    def _enqueue_protection(self, symbol: str) -> None:
        if symbol in self._protection_queued:
            return
        self._protection_queued.add(symbol)
        self._protection_queue.put(symbol)

    def _process_protection_queue(self) -> None:
        try:
            symbol = self._protection_queue.get(timeout=0.1)
        except queue.Empty:
            return
        self._protection_queued.discard(symbol)
        pos = self.state.positions.get(symbol)
        if not pos or pos.get("status") != "FILLED" or self.state.mode != "REAL":
            return
        self._attach_real_protection(symbol, pos)
        time.sleep(1)
    ...
    def _run_scan_cycle(self) -> None:
        ...
        for symbol in universe:
            ...
            if setup:
                ...
                passed = setup.confidence >= threshold
                if hasattr(self.learn_engine, "record_scan_candidate"):
                    try:
                        self.learn_engine.record_scan_candidate(setup.to_dict(), passed, threshold, "" if passed else "BELOW_ACTIVE_THRESHOLD")
                    except Exception:
                        logger.exception("record_scan_candidate gagal untuk %s", symbol)
                if passed:
                ...
                if hasattr(self.learn_engine, "record_scan_candidate"):
                    try:
                        self.learn_engine.record_scan_candidate(
                            {"pair": symbol, "timestamp": time.time() * 1000, "strategy_version": getattr(self.strategy_engine, "version", None)},
                            False, self.strategy_engine.get_active_threshold(), "NO_VALID_ENTRY_CANDIDATE"
                        )
                    except Exception:
                        logger.exception("record_scan_candidate rejection logging gagal untuk %s", symbol)
        ...
        if self.state.mode == "REAL":
            try:
                ...
                order = self.binance.place_limit_order(
                    setup.pair, side, qty, round_price(setup.entry, filters["tick_size"])
                )
    ...
    def _worker_learn(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                self._process_protection_queue()
    ...
    def _cmd_help(self, args: List[str]) -> None:
        self.telegram.send(
            ...
            "/stats - Statistik bot\n"
            "/koin - Universe coin\n"
            "/banned <COIN>|all - Ban permanen / lihat daftar\n"
            "/unban <COIN> - Hapus ban coin\n"
            "/timeout all|<COIN> - Cleanup pending/active\n"
    ...
    def _cmd_banned(self, args: List[str]) -> None:
        ...
        if symbol.lower() == "all":
            ...
            lines = ["🚫 BANNED COINS\n"]
            now = time.time()
            for coin, info in bans.items():
                if info.get("permanent"):
                    remaining = "PERMANENT"
                else:
                    remaining_sec = max(0.0, (info.get("expiry") or now) - now)
                    remaining = f"{int(remaining_sec // 3600):02d}h {int((remaining_sec % 3600) // 60):02d}m"
                lines.append(f"{coin}\nReason: {info['reason']}\nRemaining: {remaining}\n")
            self.telegram.send("\n".join(lines), "INFO")
            return
        info = bans.get(symbol)
        if not info:
            self.state.ban(symbol, "PERMANENT", permanent=True)
            self.telegram.send(f"🚫 BANNED PERMANENTLY — {symbol}", "BANNED")
            return
        remaining = "PERMANENT" if info.get("permanent") else f"{max(0, info.get('expiry', 0)-time.time()) / 3600:.1f}h"
        self.telegram.send(f"🚫 {symbol}\nReason: {info['reason']}\nRemaining: {remaining}", "INFO")

    def _cmd_ban(self, args: List[str]) -> None:
    ...
    def _cmd_unban(self, args: List[str]) -> None:
        if not args:
            self.telegram.send("⚠️ Format salah.\n\nGunakan:\n/unban BTCUSDT\natau\n/unban All", "INFO")
            return
        self.state.unban(args[0].upper())
        self.telegram.send(f"✅ UNBANNED — {args[0].upper()}", "UNBANNED")

    def _cmd_timeout(self, args: List[str]) -> None:
        if not args or (args[0].lower() != "all" and not args[0].strip()):
            self.telegram.send(
                "❌ Invalid /timeout usage.\n\nUse:\n/timeout all\n/timeout BTCUSDT",
                "INFO",
            )
            return

        targets = list(self.state.positions.keys()) if args[0].lower() == "all" else [args[0].upper()]
        cleaned: List[str] = []
        for symbol in targets:
            ...
            if self.state.mode == "REAL":
                try:
                    ...
                    if risk:
                        self.binance.close_position_market(symbol, float(risk.get("positionAmt", 0)))
                    # Verify cleanup; already-missing resources are safe/idempotent.
                    remaining_orders = self.binance.get_open_orders(symbol)
                    remaining_risk = self.binance.get_position_risk(symbol)
                    if remaining_orders or remaining_risk:
                        logger.warning("Manual timeout cleanup incomplete for %s", symbol)
                        continue
                except RateLimitError as e:
                    self._enter_binance_pause(str(e))
                    continue
                except ExchangeError as e:
                    logger.warning("Manual timeout cleanup %s: %s", symbol, e)
                    continue

            if pos and self.state.manual_cancel(symbol):
                # Administrative cleanup: never call learn.record_* here.
                self.ws.unsubscribe(symbol)
                cleaned.append(symbol)

        if cleaned:
            self.telegram.send(f"✅ Manual timeout completed: {', '.join(cleaned)}", "INFO")
        else:
            self.telegram.send("ℹ️ Tidak ada state yang berhasil dibersihkan.", "INFO")
...
async def handle_update(update: dict, context: dict = None):
    """Receive one raw Telegram update from try.py; never poll Telegram here."""
    global _LAUNCHER_BOT
    try:
        ...
        if not text.startswith("/"):
            return None
        expected = str(context.get("user_id") if context else "").strip()
        actual = str((message.get("from") or {}).get("id") or "").strip()
        allowed = str(_LAUNCHER_BOT.cfg.allowed_user_id).strip()
        if allowed and actual and actual != allowed:
            logger.warning("Command ignored from unauthorized user id=%s", actual)
            return None
        logger.info("[COMMAND BRIDGE] %s", text)
        await asyncio.to_thread(_LAUNCHER_BOT._dispatch_command, text)
    except Exception:
        logger.exception("handle_update gagal")
    return None
